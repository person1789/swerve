...
import { MonitorPlay, Wifi, WifiOff, Monitor, Radio, ChevronDown, ChevronRight, Settings2, X, Download, Route } from 'lucide-react';
  ...
  const bundles = useMemo(() => {
    ...
    if (pose) {
      processed['Robot Pose'] = {
        ...
        'Heading': pose.headingDegrees.toFixed(1) + ' deg',
      };
    }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '0.2rem 0.5rem', gap: '0.5rem' }}>
        <button onClick={onDownloadCSV} title="Download Background Telemetry History (CSV)" style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Download size={12} />
        </button>
        <button onClick={() => setShowSettings(!showSettings)} title="Custom Grouping Settings" style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Settings2 size={12} />
        </button>
      </div>
